...
const interruptionStarts = ["Wacht even.", "Maar eerlijk?", "Ja, maar luister.", "Ik ga u onderbreken.", "Dat klinkt mooi, maar"];
const extraObjections = ["ik vertrouw die berekening nog niet.", "ik wil geen druk voelen vandaag.", "ik heb slechte ervaringen met verkopers.", "ik weet niet of dit nu prioriteit heeft.", "ik wil eerst drie offertes vergelijken."];

let state = { scenario: null, pressure: false, turn: 0, lastObjection: "" };
...
pressureButton.addEventListener("click", () => {
  ...
  addMessage("customer", "Klant", state.pressure ? "Oké, dan zeg ik het direct: ik heb weinig tijd, ik wil geen verkoopspraat, en het moet financieel kloppen." : "Goed, dan wil ik vooral rustig begrijpen waarom dit voor mij zinvol zou zijn.");
});
...
function startScenario() {
  ...
  addMessage("customer", "Klant", `Ik snap wat u zegt, maar ${lowerFirst(objection)}`);
  replyInput.focus();
}
...
function handleSellerAnswer(answer) {
  ...
  addMessage("customer", "Klant", nextCustomerLine(evaluation));
}
...
function evaluate(answer) {
  ...
  return {
    ...
    feedback: buildFeedback({ questionCount, hasOpenQuestion, hasReflect, pushCount, words }),
...
function buildFeedback(details) {
  const feedback = [];
  if (!details.hasReflect) feedback.push("Je neemt de objectie te weinig eerst terug in jouw woorden.");
  if (!details.hasOpenQuestion) feedback.push("Je vraag is te gesloten of ontbreekt, waardoor jij moet gaan overtuigen.");
  if (details.pushCount > 0) feedback.push("Je pusht door zekerheid te claimen in plaats van de klant te laten nadenken.");
  if (details.words > 55) feedback.push("Je praat te lang; daardoor geef je controle weg.");
  if (!feedback.length) feedback.push("Je houdt controle, maar maak je vervolgvraag scherper en concreter.");
...
function nextCustomerLine(evaluation) {
  ...
  if (state.pressure) return `${interrupt} ${lowerFirst(base)} En bovendien: ${extra} Waarom zou ik hier nu tijd in steken?`;
  if (weakScore) return `${interrupt} ik voel nog vooral dat u mij wilt overtuigen. ${base}`;
  if (state.turn % 3 === 0) return `Dat is een betere vraag. Maar ${extra}`;
...
function addCoachMessage(evaluation) {
  ...
  wrapper.innerHTML = `<strong>Trainer</strong><div class="score-grid"><div class="score"><span>Controle</span>${evaluation.controle}/10</div><div class="score"><span>Vraagstelling</span>${evaluation.vraagstelling}/10</div><div class="score"><span>Rust/tempo</span>${evaluation.rust}/10</div></div><p><strong>Harde feedback</strong>${evaluation.feedback.map(escapeHtml).join("<br>")}</p><p><strong>Verbeterde versie</strong>${escapeHtml(evaluation.improved)}</p>`;
...
function splitObjections(value) { return clean(value).split(/\n|,/).map((item) => item.trim()).filter(Boolean); }
